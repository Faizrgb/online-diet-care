        ...
        <div className="flex flex-col lg:flex-row items-center gap-16 lg:flex-row-reverse">
          ...
          >
            <div className="bg-white p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-slate-100 relative">
              <h3 className="text-2xl font-bold text-slate-900 mb-6">Calorie Calculator</h3>
              ...
              <form onSubmit={calculateBMR} className="space-y-5">
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl">
                  <button
                    ...
                    className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${gender === "male" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                  >
                    Male
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${gender === "female" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                  >
                    Female
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="age" className="block text-sm font-semibold text-slate-700 mb-2">Age (yrs)</label>
                    <input
                      id="age"
                      type="number"
                      value={age}
                      ...
                      placeholder="e.g. 28"
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all bg-slate-50 focus:bg-white text-slate-900 font-medium placeholder:text-slate-400"
                      required
                    />
                  </div>
                  <div>
                    <label htmlFor="bmr-height" className="block text-sm font-semibold text-slate-700 mb-2">Height (cm)</label>
                    <input
                      id="bmr-height"
                      type="number"
                      value={height}
                ...
                <div>
                  <label htmlFor="bmr-weight" className="block text-sm font-semibold text-slate-700 mb-2">Weight (kg)</label>
                  <input
                    id="bmr-weight"
                    type="number"
                    value={weight}
                ...
                <div>
                  <label htmlFor="activity" className="block text-sm font-semibold text-slate-700 mb-2">Activity Level</label>
                  <select
                    id="activity"
                    value={activity}
                    onChange={(e) => setActivity(e.target.value)}
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all bg-slate-50 focus:bg-white text-slate-900 font-medium appearance-none cursor-pointer"
                  >
                ...
                <div className="pt-2 flex gap-3">
                  <button
                    type="submit"
                    className="flex-1 bg-slate-900 text-white font-semibold py-3 px-6 rounded-xl hover:bg-slate-800 transition-all shadow-md flex items-center justify-center gap-2"
                  >
              ...
              {bmr && tdee && (
                ...
                >
                  <div className="mb-4">
                    ...
                    <div className="text-3xl font-bold text-slate-900 font-serif">{bmr} <span className="text-lg text-slate-500 font-sans font-medium">kcal/day</span></div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="text-4xl font-bold text-orange-600 font-serif">{tdee} <span className="text-xl text-slate-500 font-sans font-medium">kcal/day</span></div>
                    <p className="text-sm text-slate-600 mt-2 font-medium">Eat this amount to maintain your weight.</p>
                  </div>
                </motion.div>
              )}
            </div>
